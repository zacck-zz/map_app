...
public class shar extends Activity implements OnClickListener,
		OAuthDialogListener {
	Facebook mfacebook = new Facebook("141358469324099");
	Button fb, Tw;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mUpdate = getIntent().getExtras().getString("update");
	...
	public void onClick(View v) {
		...
		case R.id.socfb:
			...
			if (!mfacebook.isSessionValid()) {
				mfacebook.authorize(shar.this, new String[] { "publish_stream",
						"publish_checkins" }, new DialogListener() {
			...
			try {
				...
				upDets.putString("message", mUpdate);
				...
				Toast.makeText(shar.this, "posted to timeline",
		...
		case R.id.soctw:
			...
			//
			WebViewOAuthDialogWrapper pageWrapper = new WebViewOAuthDialogWrapper(
					webView);
			//
		...
		try {
			if (uam.verifyCredential()) {
				TweetER.getInstance(uam).post(
						new Tweet(mUpdate + System.currentTimeMillis()));
				//
